  ...
  if (kind === "plan" || kind === "saved") {
    return (
      <main className="container mx-auto flex-1 px-4 py-6 sm:px-8">
        ...
        <div className="border border-gray-800 bg-[#10141a] p-3 sm:p-7">
          <section className="border border-gray-800 px-3 py-3">
          ...
          <section className="mt-3 grid grid-cols-3 divide-x divide-gray-800 border border-gray-800 px-3 py-4">
          ...
          <section className="mt-3 border border-gray-800">
            ...
            <div className="space-y-2 p-2">
              {items.map((item) => {
                ...
                return (
                  <div key={id} className="flex flex-wrap items-center gap-2 border border-gray-800 bg-[#151a21] p-2 sm:flex-nowrap sm:gap-4">
                    ...
                    <Link href={`/workout/${id}`} className="min-w-[120px] flex-1">
                    ...
                    {kind === "plan" ? <button onClick={() => setCompleted((current) => isDone ? current.filter((entry) => entry !== id) : [...current, id])} className={`whitespace-nowrap rounded-full px-2 py-1.5 text-[10px] font-bold sm:px-3 ${isDone ? "bg-gray-700 text-white" : "bg-[#ccff00] text-black hover:bg-[#b8e600]"}`}>{isDone ? "Done" : "Mark as Done"}</button> : null}
